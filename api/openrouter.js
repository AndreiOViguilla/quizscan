...
module.exports = async function handler(req, res) {
  ...
  try {
    const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "meta-llama/llama-3.1-8b-instruct:free", max_tokens: maxTokens, messages }),
    });
    const data = await r.json();
    if (data.error) return res.status(502).json({ error: data.error.message || "OpenRouter error" });
    const content = data.choices?.[0]?.message?.content || "";
    if (!content) return res.status(502).json({ error: "Empty response from OpenRouter" });
    ...
    return res.status(502).json({ error: err.message || "OpenRouter unreachable" });
  }
};
